...
## Owner kernel-module signing

Source-kernel control builds can use one stable local module trust anchor
instead of the kernel build's autogenerated key. Create or verify it on
`l-esp` or `l-portal`:
...
This creates two SOPS-encrypted documents. The shared document contains only
the public RSA-4096 X.509 certificate and its digest; `l-esp`, `l-portal` and
the optional remote builder can decrypt it. The private document contains the
matching PEM key and is restricted to `l-esp` and `l-portal`. The certificate
has a long validity window, but its digest—not a public CA—is the pinned local
identity.

Compilation embeds only that public certificate and deliberately leaves the
eight stripped control modules unsigned:

```bash
# Safe on the optional compilation host.
nix run .#kernel-module-owner-build -- \
  --output ./result-karen-owner-kernel

# Add --adb-public-key /private/path/adbkey.pub for a key-bound recovery.
```

After returning the bundle to an owner host, apply the private signature:

```bash
nix run .#kernel-module-owner-sign -- \
  ./result-karen-owner-kernel \
  ./result-karen-owner-kernel-signed
```

The signing helper requires the exact fixed-key effective config, checks that
the exported public certificate matches both SOPS halves, refuses already
signed or unexpected modules and verifies the unsigned payload manifest before
signing. It retains separate unsigned and signed SHA-256 manifests without
ever exporting the private key. Identical unsigned manifests prove
reproducibility between owner builds; equality with stock is not required
because the compiler and reviewed source/config changes differ. The stable
certificate removes build-time key randomness from the kernel link and keeps
subsequent compiler-cache entries reusable. Generic builds still use their
isolated autogenerated key and need no repository secret.
